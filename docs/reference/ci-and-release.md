...
- `ci.yml` runs on pushes and pull requests. It restores in locked mode, checks Fantomas formatting, builds on Ubuntu and Windows, runs non-package tests, enforces exact branch-coverage gates on Ubuntu (Core >=80%, MAF adapter >=80%), builds docs, packs once on Ubuntu, runs package smoke tests, validates SourceLink, scans for vulnerable and deprecated packages, and verifies the working tree stays clean.
- `maf-compat.yml` runs on a nightly schedule or by hand. It resolves the newest `Microsoft.Agents.AI` `1.x` version from NuGet, overrides the central package version for that run, executes the MAF adapter test suite, and uploads a compatibility report artifact. It does not open issues automatically.
...
Core and MAF branch coverage are gated at 80% using the exact `branches-covered / branches-valid` totals in Coverlet Cobertura reports. The Core command uses Coverlet's default metric with no attribute or source exclusions, so its denominator includes instrumentable F# task and async-enumerator state-machine branches alongside other authored and generated branches. The workflows do not substitute a narrower source-presence proxy for that report-wide ratio.

The MAF adapter retains its longstanding test-project `CompilerGeneratedAttribute` setting and historical metric. That project-level configuration is unchanged by the Core gate; neither workflow adds or broadens an exclusion.
