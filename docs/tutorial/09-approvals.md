...
Pause a sensitive tool call for a host decision. This project is a compiling chapter skeleton; its complete lesson will replace the placeholder in the next tutorial-writing pass.
...
From the repository root, open `tutorials/fsharp/09-approvals`. Live chapters will require explicit provider environment variables; chapter 16 remains offline.
...
```bash
dotnet run --project tutorials/fsharp/09-approvals
```

Representative placeholder output (the completed live chapter's provider-generated values will be variable):

```text
Chapter 9 will build on the support-ticket agent.
```

## What changed

This skeleton reserves chapter 9's approved project and documentation boundary. The completed lesson will explain its single delta from chapter 8.
