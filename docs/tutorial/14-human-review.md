...
Approve or reject a workflow on its live handle. This project is a compiling chapter skeleton; its complete lesson will replace the placeholder in the next tutorial-writing pass.
...
From the repository root, open `tutorials/fsharp/14-human-review`. Live chapters will require explicit provider environment variables; chapter 16 remains offline.
...
```text
Chapter 14 will build on the support-ticket agent.
```

## What changed

This skeleton reserves chapter 14's approved project and documentation boundary. The completed lesson will explain its single delta from chapter 13.
